...
As a developer using agentctl, I want my custom slash commands to sync to each AI agent that supports them, so I can reuse consistent workflows across tools.
...
- Sync is one-way from agentctl configuration to each agent's command configuration; importing existing agent commands is out of scope.
