...
`agents/.local/` or via `.local` filename suffixes, with predictable sync
behavior and safe ignore guidance.
...
- If a local item exists in both `agents/.local/` and as a `.local` filename
  suffix, the `agents/.local/` version wins.
