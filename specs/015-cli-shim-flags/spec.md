...
- **FR-022**: When invoking an agent, the shim MUST pass through agent output unmodified for every `--output` mode (including `json` and `stream-json`) and MUST NOT wrap or reformat it.
